...
def predict_dyslexia(features):
    """
    Predicts dyslexia given the processed input features.
    Args:
        features (list or np.array): Feature list
    
    Returns:
        dict: Prediction and probability score
    """
    features = np.array(features, dtype=float).reshape(1, -1)
    
    # Make prediction
    prediction = model.predict(features)
    probability = model.predict_proba(features)[:, 1]  # Probability of dyslexia (class 1)
...
def predictDyslexia(data:InputData1):
    ...
    return model1.predict_dyslexia(features)[0]
